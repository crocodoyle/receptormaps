...
results_dir = 'E:/brains/receptor_map_results/'
...
for filename in os.listdir(data_dir):
    ...
    hemisphere = tokens[3]
    receptor = tokens[4]
    baking_sheet = tokens[5]
    scan = tokens[6][:-4]

    img = np.asarray(Image.open(data_dir + filename), dtype='uint8')
    ...
    plt.savefig(results_dir + receptor + '_' + scan + '_histograms.png', bbox_inches='tight')

    # print('global threshold found with otsu method:', global_thresh)
    thresholded_low = (img > 50) * img
    clipped = (thresholded_low < 175) * img
    ...
    plt.savefig(results_dir + receptor + '_' + scan + '_thresholded.png', bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    plt.axis('off')
    plt.savefig(results_dir + receptor + '_' + scan + '_rescaled.png')
    plt.clf()
